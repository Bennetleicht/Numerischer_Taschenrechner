...
class LRGUI(tk.Tk):
    def __init__(self):
        ...
        self.stepper: Optional[LRDecompositionStepper] = None
        self.step_count = 0
        self.started = False
    ...
    def _build_ui(self):
        ...
        ttk.Label(head, text="LR-Zerlegung", style="Title.TLabel").pack(side="left")
    ...
    def _append_step_card(self, snap: Dict[str, Any], step: Step, extra_title: Optional[str] = None):
        ...
        frame_L = ttk.Frame(lr_row, style="Card.TFrame")
        frame_R = ttk.Frame(lr_row, style="Card.TFrame")
        frame_L.pack(side="left", anchor="n", padx=(0, 18))
        frame_R.pack(side="left", anchor="n")

        ttk.Label(frame_L, text="L", style="Sub.TLabel").pack(anchor="w")
        ttk.Label(frame_R, text="R", style="Sub.TLabel").pack(anchor="w")

        self._draw_matrix(frame_L, "L", snap["L"], changed, pivot_r, pivot_c, step)
        self._draw_matrix(frame_R, "R", snap["R"], changed, pivot_r, pivot_c, step)
    ...
    def _draw_matrix(self, parent: ttk.Frame, name: str, M: List[List[float]],
        ...
        for r in range(len(M)):
            for c in range(len(M[0])):
                ...
                if step.kind == "lr_update" and name == "R" and r == pivot_r and c == pivot_c:
                    bg = self.bg_pivot
    ...
    def on_start_or_next(self):
        ...
        self.stepper = LRDecompositionStepper(A, b, change_tol=1e-10)
        self.started = True
