...
from jacobi_methode import JacobiStepper
from jacobi_solver import JacobiRowDetail, Step as JacobiStep
...
class JacobiGUI(tk.Tk):
    def __init__(self, parent=None):
        ...
        self.stepper: Optional[JacobiStepper] = None
        self.started = False
        self.step_count = 0
    ...
    def _build_ui(self):
        ...
        ttk.Label(head, text="Jacobi-Verfahren", style="Title.TLabel").pack(side="left")
    ...
    def _build_input_matrix(self):
        ...
        ttk.Label(right, text="Jacobi-Formel", style="Sub.TLabel").pack(anchor="w", pady=(0, 8))
    ...
    def _append_colored_math_row(self, parent, detail: JacobiRowDetail, k_old: int, k_new: int, row: int):
        ROW_H = 28
        PADY_BOX = 3
        ...
        for idx, (j, aij, _xj, _prod) in enumerate(detail.terms):
            if idx > 0:
                draw_txt("  +  ")
            draw_box(self._fmt(aij), self.bg_offdiag)
            draw_txt(" · ")
            draw_txt(self._x_symbol(j + 1, k_old))
    ...
    def _append_step_card(self, A: List[List[float]], b: List[float], step: JacobiStep):
        self.step_count += 1
        n = len(A)
    ...
    def _append_final_solution_card(self):
        ...
        header = ttk.Label(card, text="LÖSUNG / NÄHERUNG", style="Header.TLabel", padding=(8, 6))
        header.pack(fill="x")
        ...
        messagebox.showinfo("Jacobi-Ergebnis", sol)
    ...
    def on_start_or_next(self):
        ...
        try:
            ...
            self.stepper = JacobiStepper(A, b, x0, tol=tol, safety_limit=100)
